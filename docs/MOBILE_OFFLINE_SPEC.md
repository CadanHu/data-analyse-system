...
### 4.4 手机端本地生成深度洞察看板策略 (Local Report)

针对“✨ 生成深度洞察看板”功能，手机离线/直连模式与电脑 Web 端采取了不同的隔离策略：
- **Web 端（高负荷）**：走后台 `KnowledgeExtractionService` 的 Map-Reduce 多线程并行处理，提供详尽的全历史级上下文供模型拆解分析。
- **手机端（轻量化）**：为了防止手机网络直连时单次长连接超时、崩溃或大量消耗 Token，采用 `localReportService.ts` 单次直出的克制策略。
  - **遇到问题**：此前因极端缩减，提示词强限制“最多生成 2 个图表”且仅保留“前 20 条数据”，导致手机生成的报告过于单薄，无法给用户提供完整的洞察价值。
  - **当前规格调整**：
    1. **放宽模型图表数量限制**：提示词中要求模型**“根据数据维度，有逻辑地生成 3-5 个不同类型的专业图表模块”**，提升报告视觉和业务深度。
    2. **扩充上下文数据源**：将喂给直连模型的数据切片从前 20 行提升至 **前 50 行**，赋予 AI 更全面的特征提取样本，同时保证请求不会超限。
