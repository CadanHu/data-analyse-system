...
- **candlestick**: 用于展示【股价波动、价格区间】。SQL 必须按顺序 SELECT 日期列、`open_price AS open`、`close_price AS close`、`low_price AS low`、`high_price AS high`，前端依赖这些别名渲染K线。
- **map**: 当用户明确要求【地理地图、城市分布、地区热力图】时选择。SQL **必须**只按 `c.city` 分组（`GROUP BY c.city`），用 `AVG(c.latitude) AS latitude`、`AVG(c.longitude) AS longitude` 取城市平均坐标（同一城市不同客户坐标略有差异，若按 city+lat+lng 分组会产生大量重复点导致地图标签密集重叠），同时 SELECT 一个数值指标字段（如销售额、订单量）。
...
2. **Status Filtering**: Use `column IN ('Shipped', 'shipped')` or exact match. **Never** use `LOWER(column) LIKE '%value%'` — it prevents index usage and causes full-table scans that time out.
...
【Chart Type Rules】
- **candlestick**: For stock/price range analysis. SQL MUST SELECT date column, `open_price AS open`, `close_price AS close`, `low_price AS low`, `high_price AS high` in that order — frontend renders K-line using these aliases.
- **map**: When user asks for geographic/city distribution. SQL MUST SELECT `c.city`, `c.latitude`, `c.longitude` plus a metric, via JOIN customers.
